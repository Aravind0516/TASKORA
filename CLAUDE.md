...
TASKORA is a real multi-tenant internal company project/task management platform, being upgraded incrementally (see `TASKORA_AUDIT.md` for the current gap analysis and phase plan) toward the fuller company-workflow spec. Firebase is fully wired: Authentication (email/password + an invitation-based onboarding flow) and Firestore (`organizations`, `users`, `teams`, `projects`, `tasks`, `invitations`, `notifications`, `activityLogs`, `projectDeliverables` collections, among others), all scoped by `organizationId` and enforced by claims-based, default-deny Firestore rules — not a mock-data prototype. Every module reads/writes real Firestore data live via `onSnapshot`. `lib/mock-data/*` remains in the repo as inert reference/history and is not imported by any live view **except** `lib/mock-data/platform-data.ts`'s `systemServices` export (imported by `components/platform/platform-provider.tsx` and shown, unchanged, on the Super Admin `system`/`analytics`/`overview` views as a static platform-service-status display — there is no real infrastructure-monitoring integration behind it). The project detail page's "Deliverables" tab was migrated off static mock data to a real `projectDeliverables/{id}` collection (see `lib/services/deliverable.service.ts`) during the final pre-production audit fix pass. Read `AGENTS.md` before touching Next.js APIs/conventions — it is auto-maintained by `next dev` and flags breaking changes versus older Next.js knowledge; commit it along with your other changes.
...
- Three system roles, set exclusively server-side (Admin SDK — a client can never set its own claims): `super_admin` (platform owner, provisioned only via `scripts/bootstrap-super-admin.mjs`, never public registration), `admin` (an organization's owner — granted only when a Super Admin approves that account's `organizationRegistrationRequests/{id}` document, or via an invited admin), `user` (an org member, always arrives via accepting an invitation). A public "Get Started" self-registration always creates role `user` with `organizationId: null`, enforced by `firestore.rules`'s `users/{uid}` create rule regardless of what the client sends.
- **Registration is choice-first, never organization-first**: `/register` (`components/auth/register-flow.tsx`) starts with a Join-vs-Register choice (`RegisterChoiceStep`) — "Join an existing organization" is pure information pointing back to the invitation flow (never creates an account or a request); "Register a new organization" creates the Firebase account (role `user`, `organizationId: null`, unchanged) and then submits an `organizationRegistrationRequests/{id}` document (`lib/server/organization-registrations.ts`) for Super Admin review — never an active organization, never admin claims. Only `approveOrganizationRegistration()` (Super Admin only, `/superadmin/organization-requests`) ever creates the real organization and grants admin, and only to that request's own `createdUserId`. The OLD instant self-serve claim (`POST /api/organizations/self-serve`, `claimNewOrganizationForSelf()`) is still present and still works exactly as before, but is no longer called by any UI path — left in place rather than deleted since it was working, tested code with no other known caller. An org-less signed-in account with a pending/rejected registration request sees a dedicated state (`components/auth/organization-registration-gate.tsx`, mounted in `AppShell`) instead of an empty workspace dashboard.
